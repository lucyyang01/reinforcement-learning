...
class PerceptronModel(object):
    ...
    def train(self, dataset):
        ...
        while (True):
            error = 0
            # for data in dataset.iterate_once(batch_size): #wrap in while loop?
            for x, y in dataset.iterate_once(batch_size):
                #classify w current weights
                predicted_y = self.get_prediction(x)
                actual_y = nn.as_scalar(y)
                
                if (predicted_y != actual_y):
                    error += 1
                    #returns out of the loop
                    self.w.update(x,actual_y)
            if error == 0:
                break
...
class RegressionModel(object):
    ...
    def get_loss(self, x, y):
        ...
        """
        "*** YOUR CODE HERE ***"
        #return the square loss of the predicted y and x;
